Logging = {
    limit = 5,
    level = 0,
    debug_values = { none = 0, error = 1, warn = 2, info = 3, debug = 4, trace = 5 }
}
        ...
        if object.valid then
            ...
            if help ~= nil and help ~= "" then
                ...
                elseif string.find(lua_type, "Lua") then
                    ...
                    message = message ..
                    string.format("{\"type\":%q,\"name\":%q}", object.type or "nil", object.name or "nil")
                end
            else
...
function Logging:logging(tag, level, ...)
    ...
    self:writer(string.format("%s %s:%s|%s", tag, string.match(debug_info.source, "[^/]*$"), debug_info.currentline,
        message))
end
